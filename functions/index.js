...
// // httpsCallable serializes Firestore Timestamp objects into plain
// // { seconds, nanoseconds } maps during transit. This helper walks the
// // notifications array and reconstructs any date_* fields back into
// // native admin.firestore.Timestamp instances before writing to Firestore.
// function reconstructNotificationDates(categoryData) {
//   if (!Array.isArray(categoryData.notifications)) return categoryData;
//   const langs = ['sor', 'bad', 'ar', 'en'];
//   const notifications = categoryData.notifications.map(notif => {
//     const fixed = { ...notif };
//     langs.forEach(l => {
//       const key = `date_${l}`;
//       const val = fixed[key];
//       if (val && typeof val === 'object' && typeof val.seconds === 'number') {
//         fixed[key] = new admin.firestore.Timestamp(val.seconds, val.nanoseconds || 0);
//       }
//     });
//     return fixed;
//   });
//   return { ...categoryData, notifications };
// }
...
//   const { id } = data;
//   // Reconstruct notification date_* fields from serialized { seconds, nanoseconds } back to Timestamps
//   const categoryData = reconstructNotificationDates(data.categoryData);
//   try {
//     if (id) {
//       await db.collection('categories_new').doc(id).update({ ...categoryData, updatedAt: admin.firestore.FieldValue.serverTimestamp() });
//       return { success: true, id, action: 'updated' };
...
// Reconstruct Firestore-native types from plain serialized objects sent via httpsCallable.
// httpsCallable JSON-serializes everything, so GeoPoints become {lat,lng}, Timestamps become
// {seconds,nanoseconds}, and DocumentReferences become plain strings. This function converts
// them back to proper Admin SDK types before writing to Firestore.
const LANGS = ['sor', 'bad', 'ar', 'en'];
function reconstructServiceData(serviceData) {
  const d = { ...serviceData };

  // categoryref: plain string ID → DocumentReference
  if (d.categoryref && typeof d.categoryref === 'string') {
  ...
  LANGS.forEach(l => {
    ...
    if (ll && typeof ll === 'object' && ll.lat != null && ll.lng != null) {
      d[`latlng_${l}`] = new admin.firestore.GeoPoint(ll.lat, ll.lng);
    }

    // createddate / expiredate: {seconds, nanoseconds} → Timestamp
    ['createddate', 'expiredate'].forEach(field => {
      const key = `${field}_${l}`;
      const val = d[key];
      if (val && typeof val === 'object' && typeof val.seconds === 'number') {
        d[key] = new admin.firestore.Timestamp(val.seconds, val.nanoseconds || 0);
      }
    });
  });
...
exports.createOrUpdateService = onCall(CORS_CONFIG, async (request) => {
  ...
  const { id } = data;
  // Reconstruct GeoPoint, Timestamp, and DocumentReference from plain serialized objects
  const serviceData = reconstructServiceData(data.serviceData);
  try {
    if (id) {
      await db.collection('services_new').doc(id).update({ ...serviceData, updatedAt: admin.firestore.FieldValue.serverTimestamp() });
      return { success: true, id, action: 'updated' };
