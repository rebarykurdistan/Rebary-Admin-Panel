...
//   useEffect(() => { filterServices(); }, [searchTerm, categoryFilter, services]);
...
//     }
//     if (categoryFilter) {
//       filtered = filtered.filter(s => s.categoryref === categoryFilter);
//     }
//     setFilteredServices(filtered);
//   };
...
//                 {searchTerm && (
//                   <button onClick={() => setSearchTerm('')} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600"><FiX size={16} /></button>
//                 )}
//               </div>
//               <div className="w-full lg:flex-1">
...
//                   <option value="">All Categories</option>
//                   {categories.map(cat => (
//                     <option key={cat.id} value={`categories_new/${cat.id}`}>{cat.name_sor || cat.name_en || cat.id}</option>
//                   ))}
//                 </select>
//               </div>
//               {selectedServices.length > 0 && (
...
//     }
//     if (categoryFilter) {
//       // categoryFilter is a string like "categories_new/ID"
//       // s.categoryref may be a DocumentReference or string — normalize both
//       const filterCatId = categoryFilter.includes('/') ? categoryFilter.split('/').pop() : categoryFilter;
//       filtered = filtered.filter(s => {
//         const ref = s.categoryref;
//         if (!ref) return false;
//         if (typeof ref === 'object' && ref.id) return ref.id === filterCatId;
//         if (typeof ref === 'object' && ref.path) return ref.path.split('/').pop() === filterCatId;
//         if (typeof ref === 'string') return (ref.includes('/') ? ref.split('/').pop() : ref) === filterCatId;
//         return false;
//       });
//     }
//     setFilteredServices(filtered);
...
//     }
//     if (categoryFilter) {
//       // categoryFilter is now just the raw category ID
//       filtered = filtered.filter(s => {
//         const ref = s.categoryref;
//         if (!ref) return false;
